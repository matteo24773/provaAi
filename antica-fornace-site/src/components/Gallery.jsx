  ...
  return (
    <section id="gallery" className="section bg-bgLight relative">
      <div className="container-custom">        <div className="text-center mb-12">
          <h2 className="text-3xl md:text-4xl font-dragonball text-dbYellow mb-4 relative inline-block">
            Galleria del Super Saiyan
            <div className="absolute -top-3 -right-6 w-8 h-8 dragon-star animate-dragonball-glow"></div>
          </h2>
          <p className="text-lg text-gray-700 max-w-2xl mx-auto">
            Esplora le nostre <span className="text-primary font-dragonball">creazioni potentissime</span> e lasciati ispirare dai sapori 
            <span className="text-dbYellow font-dragonball"> da livello 9000</span> italiani.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {galleryImages.map((image, index) => (
            <div 
              key={index} 
              className="dragonball-card cursor-pointer group overflow-hidden"
              onClick={() => openLightbox(index)}
            >
              <div className="h-8 bg-primary flex items-center justify-center">
                <h3 className="text-white text-sm font-dragonball">{image.title}</h3>
              </div>
              <div className="relative overflow-hidden">
                ...
                {/* Dragon Ball effect on hover */}
                <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                  <div className="dragon-star w-12 h-12 animate-dragonball-glow"></div>
                </div>
              </div>
              <div className="h-6 bg-dbYellow text-xs text-center text-black font-dragonball py-1">
                PIZZA-{(index + 1).toString().padStart(2, '0')}
              </div>
            </div>
          ))}
        </div>
